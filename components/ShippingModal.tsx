...
type Props = {
  ...
  // ✅ (추가) 이 모달에서 "주문수량" 표시용
  // OrderDetail에서 해당 row의 주문수량(EA)을 넘겨주면 됨
  orderEaQty?: number;
...
export function PalletDirectTransferModal({
  ...
  // ✅ (설정모드) 선택된 파렛트의 EA 합계
  const selectedTransferEaQty = useMemo(() => {
    if (selectedPalletIds.length === 0) return 0;
    const map = new Map(DEMO_PALLETS.map((p) => [p.id, p] as const));
    return selectedPalletIds.reduce((sum, id) => sum + num(map.get(id)?.eaQty), 0);
  }, [selectedPalletIds]);

  // ✅ (현황모드) 이송중 파렛트 EA 합계
  const statusTransferEaQty = useMemo(() => {
    return statusPalletDetails.reduce((sum, p) => sum + num(p.eaQty), 0);
  }, [statusPalletDetails]);

  // ✅ 주문수량(EA) 우선순위:
  //   1) props.orderEaQty (OrderDetail에서 넘겨준 값)
  //   2) existingTransfer.orderEaQty (저장돼 있으면)
  const displayOrderEaQty = useMemo(() => {
    if (typeof orderEaQty === "number") return orderEaQty;
    if (existingTransfer?.orderEaQty != null) return existingTransfer.orderEaQty;
    return undefined;
  }, [orderEaQty, existingTransfer]);

  // ✅ 지정이송 수량(EA) 우선순위:
  //   1) 현황모드: statusTransferEaQty
  //   2) 설정모드: selectedTransferEaQty
  const displayTransferEaQty = isStatusMode ? statusTransferEaQty : selectedTransferEaQty;

  // ✅ 잔량(EA)
  const displayRemainEaQty =
    displayOrderEaQty == null ? undefined : Math.max(displayOrderEaQty - displayTransferEaQty, 0);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="flex h-[700px] w-[980px] flex-col rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex flex-1 gap-4 overflow-hidden px-5 py-4 text-[11px]">
            ...
            {!isStatusMode && (
              <div className="flex flex-1 flex-col rounded-xl border bg-gray-50/80 p-3">
                      ...
                      <tr>
                        <th className="w-8 p-1 text-center">
                          <input
                            ...
                            onChange={(e) =>
                              setSelectedPalletIds(
                                e.target.checked ? palletsForProduct.map((p) => p.id) : [],
                              )
                            }
                          />
                        </th>
                ...
                <div className="mt-2 grid grid-cols-2 gap-2">
                  <StatCard
                    label="선택된 파렛트"
                    value={`${selectedPalletIds.length}개`}
                    sub="파렛트 개수"
                  />
                  <StatCard
                    label="지정이송 수량"
                    value={`${selectedTransferEaQty.toLocaleString()} EA`}
                    sub="선택 파렛트 EA 합계"
                  />
                </div>
              </div>
                  ...
                  <div key={zone}>
                    ...
                    <div className="inline-grid grid-cols-4 gap-2 rounded-xl bg-gray-50 p-3">
                      {slotsByZone[zone].map(({ id, occupied }) => {
                        const base = "flex h-9 w-9 items-center justify-center rounded-md";
          ...
          {/* 오른쪽: 현황/안내 패널 (✅ 여기 UI/수량 표시 강화) */}
          <div className="w-72 flex-shrink-0 rounded-xl border bg-gray-50 p-3 text-[11px] text-gray-700">
            {isStatusMode && existingTransfer ? (
              <>
                <p className="mb-2 text-xs font-semibold text-gray-800">지정이송 현황</p>

                <div className="grid grid-cols-1 gap-2">
                  <StatCard
                    label="현재 상태"
                    value={existingTransfer.status}
                    sub={existingTransfer.fromLocation ?? "2·3층 창고 → 1층 입출고"}
                  />
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <StatCard
                      label="주문수량"
                      value={
                        displayOrderEaQty == null
                          ? "-"
                          : `${displayOrderEaQty.toLocaleString()}`
                      }
                      sub="EA"
                    />
                    <StatCard
                      label="지정이송"
                      value={`${displayTransferEaQty.toLocaleString()}`}
                      sub="EA"
                    />
                    <StatCard
                      label="잔량"
                      value={
                        displayRemainEaQty == null
                          ? "-"
                          : `${displayRemainEaQty.toLocaleString()}`
                      }
                      sub="EA"
                    />
                  </div>
                ...
                <div className="mt-3 rounded-xl border bg-white p-2">
                  <div className="mb-1 text-[11px] font-semibold text-gray-800">
                    이송중 파렛트 ({statusPalletDetails.length}개)
                  </div>
                  <ul className="max-h-36 list-none space-y-1 overflow-auto pl-0">
                    {statusPalletDetails.map((p) => (
                      <li key={p.id} className="rounded-md border bg-white px-2 py-1">
                        ...
                        <div className="mt-0.5 text-[11px] text-gray-500">
                          {p.eaQty.toLocaleString()} EA · {p.fromLocation} · {p.lotNo}
                        </div>
                      </li>
                    ))}
                  </ul>
                ...
                <div className="mt-3 rounded-xl border bg-white p-2">
                  <div className="mb-1 text-[11px] font-semibold text-gray-800">
                    도착 위치 ({existingTransfer.destinationSlots.length}칸)
                  </div>
                  <div className="flex flex-wrap gap-1">
                    {existingTransfer.destinationSlots.map((slot) => (
                      <span
                        key={slot}
                        className="rounded-full border bg-gray-50 px-2 py-0.5 text-[11px] text-gray-700"
                      >
                        {slot}
                      </span>
                    ))}
              ...
              <>
                <p className="mb-2 text-xs font-semibold text-gray-800">지정이송 안내</p>
                <ul className="list-disc space-y-1 pl-4 text-gray-600">
                  <li>왼쪽에서 2·3층에 있는 파렛트를 선택합니다.</li>
                  <li>
                    아래 A/B/C/D zone에서 빈 칸을 선택해
                    <br />
                    1층 입출고 위치를 지정합니다.
                  </li>
                  <li>
                    선택한 파렛트 개수와 위치 개수가 동일해야
                    <br />
                    이송 버튼이 활성화됩니다.
                  </li>
                </ul>
                ...
                <div className="mt-3 grid grid-cols-1 gap-2">
                  <StatCard label="선택된 파렛트" value={`${selectedPalletIds.length}개`} />
                  <StatCard label="선택된 도착 위치" value={`${selectedSlots.length}칸`} />
                  <StatCard label="지정이송 수량" value={`${selectedTransferEaQty.toLocaleString()} EA`} />
                </div>
              </>
            )}
          </div>
